...
클래스 하이퍼엣지 $\{e\_k \in \mathcal{E}\_{\mathcal{C}}\}$의 형태 기술자로 **등주비(isoperimetric ratio)** $\phi(e\_k) = 4\pi \cdot \text{Area}(\mathbf{M}\_k) / (\text{Perimeter}(\mathbf{M}\_k))^2$를 사용하고, Z-score 기반의 softmax 가중 합산으로 점수화한다:
...
HPE를 통과한 예측에도 특정 클래스 수준의 아티팩트(환각)가 남아 있을 수 있다. SAP는 $N\_{\text{ens}}$개의 교사 예측에서 각 클래스의 **구조적 불안정성 점수**를 변동 계수로 정량화한다:
...
- **전체 손실:** $\mathcal{L}\_{\text{total}} = \mathcal{L}\_{\text{sup}} + \gamma\_{\text{unsup}} \mathcal{L}\_{\text{unsup}}$
...
- **타당성 융합 가중치** $\alpha$와 **이상 임계값** $\theta\_A$는 명확한 최적 영역을 보임
- **순도 임계값** $\tau\_p$가 엄격해질수록, 배치 크기가 커질수록 성능이 일관되게 향상
