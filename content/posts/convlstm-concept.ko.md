...
> **요약**: 순차적 데이터(Sequence Data) 처리를 위한 **Vanilla RNN**의 동작 연산과 BPTT 기울기 소멸 한계부터, 이를 해결하기 위한 **LSTM**의 Cell State 제어 메커니즘, 구조를 경량화한 **GRU**, 그리고 3D 텐서 기반 Convolution 연산을 결합하여 시공간(Spatiotemporal) 데이터 처리를 가능하게 한 **ConvLSTM**의 아키텍처를 순차적으로 분석합니다.
...
시퀀스 길이 $(t - k)$가 증가할 때 $W_{hh}$의 고윳값이 1보다 작으면 기울기가 0으로 지수적으로 수렴하는 **기울기 소멸(Vanishing Gradient)**이 발생하여 장기 의존성 학습이 불가능해집니다.
...
### 3.1 아키텍처 구조
LSTM은 정보의 지속적 전달을 전담하는 **셀 상태(Cell State, $C_t$)**를 도입하고, 3개의 게이트(Forget, Input, Output Gate)를 통해 정보의 추가 및 삭제를 제어합니다.

![](/images/convlstm/lstm_architecture.jpeg)
*Figure 2: LSTM 셀의 내부 아키텍처 및 3가지 게이트 구조*

### 3.2 게이트별 연산 메커니즘
1. **망각 게이트 (Forget Gate, $f_t$)**: 이전 셀 상태 $C_{t-1}$에서 보존할 정보의 비율(0~1)을 결정합니다.
   $$f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)$$
2. **입력 게이트 (Input Gate, $i_t$)**: 현재 입력 $x_t$로부터 셀 상태에 새로 추가할 정보의 양을 제어합니다.
   $$i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$$
   $$\tilde{C}_t = \tanh(W_c \cdot [h_{t-1}, x_t] + b_c)$$ *(새로운 후보 셀 상태)*
3. **셀 상태 갱신 (Cell State Update, $C_t$)**: 이전 셀 상태를 망각 게이트로 스케일링하고 새로운 정보를 더합니다.
   $$C_t = f_t \circ C_{t-1} + i_t \circ \tilde{C}_t$$
4. **출력 게이트 (Output Gate, $o_t$ & $h_t$)**: 갱신된 셀 상태를 바탕으로 외부 출력 은닉 상태 $h_t$를 계산합니다.
...
### 4.2 게이트별 연산 메커니즘
1. **리셋 게이트 (Reset Gate, $r_t$)**: 이전 은닉 상태 $h_{t-1}$의 정보를 새로운 후보 상태 계산 시 얼마나 반영할지 결정합니다.
   $$r_t = \sigma(W_r \cdot [h_{t-1}, x_t] + b_r)$$
2. **업데이트 게이트 (Update Gate, $z_t$)**: 이전 은닉 상태 $h_{t-1}$와 새로운 후보 상태 $\tilde{h}_t$ 간의 보간 비율을 결정합니다. (LSTM의 Forget/Input Gate 역할 통합)
   $$z_t = \sigma(W_z \cdot [h_{t-1}, x_t] + b_z)$$
3. **후보 은닉 상태 (Candidate Hidden State, $\tilde{h}_t$)**:
   $$\tilde{h}_t = \tanh(W \cdot [r_t \circ h_{t-1}, x_t] + b)$$
4. **최종 은닉 상태 갱신 (Hidden State Update, $h_t$)**:
...
1. **공간 국소성(Spatial Locality) 파괴**: 행/열 기반 픽셀 인접 관계 정보가 손실됩니다.
2. **파라미터 비효율성**: 모든 픽셀 노드가 전 결합(Fully-Connected)되어 입력 차원에 비례하여 가중치 파라미터와 연산량이 비대해집니다.
...
### 6.1 3D 텐서 기반 입출력 구조
ConvLSTM은 기존 LSTM의 행렬 곱셈 연산을 **Convolution 연산($*$)**으로 대체하여, 입력, 셀 상태, 은닉 상태 및 모든 게이트를 **3D 텐서 (Channels $\times$ Height $\times$ Width)** 형태로 유지합니다.
...
- **입력 게이트**: $$i_t = \sigma(W_{xi} * \mathcal{X}_t + W_{hi} * \mathcal{H}_{t-1} + W_{ci} \circ \mathcal{C}_{t-1} + b_i)$$
- **망각 게이트**: $$f_t = \sigma(W_{xf} * \mathcal{X}_t + W_{hf} * \mathcal{H}_{t-1} + W_{cf} \circ \mathcal{C}_{t-1} + b_f)$$
- **셀 상태 갱신**: $$\mathcal{C}_t = f_t \circ \mathcal{C}_{t-1} + i_t \circ \tanh(W_{xc} * \mathcal{X}_t + W_{hc} * \mathcal{H}_{t-1} + b_c)$$
- **출력 게이트**: $$o_t = \sigma(W_{xo} * \mathcal{X}_t + W_{ho} * \mathcal{H}_{t-1} + W_{co} \circ \mathcal{C}_t + b_o)$$
- **은닉 상태 갱신**: $$\mathcal{H}_t = o_t \circ \tanh(\mathcal{C}_t)$$

> **Convolution 커널 크기의 영향**:
...
시공간 시퀀스 예측(예: 초단기 강수 예측, 비디오 프레임 예측)을 위해 ConvLSTM 레이어를 다층으로 쌓아 **Encoding-Forecasting** 구조를 형성합니다.
...
1. **Encoding Network**: 과거 연속 이미지 시퀀스를 차례로 입력받아 숨겨진 3D 상태 텐서(Hidden/Cell State)에 시공간 특성을 압축 축적합니다.
2. **Forecasting Network**: Encoding Network의 최종 은닉 상태를 전달받아 미래 N개 시점의 3D 텐서 상태를 펼치면서 연속된 미래 이미지를 생성 및 예측합니다.
...
| :--- | :--- | :--- | :--- | :--- |
| **Vanilla RNN** | 1D Vector | Fully-Connected | 게이트 없음 (단일 Tanh) | 시간 단기 의존성만 반영 (BPTT 한계) |
| **LSTM** | 1D Vector | Fully-Connected | Forget, Input, Output Gate / **Cell State 분리** | 시간 장기 의존성 제어 (공간 정보 파괴) |
| **GRU** | 1D Vector | Fully-Connected | Reset, Update Gate / **Hidden State 통합** | 시간 장기 의존성 제어 및 경량화 (공간 정보 파괴) |
| **ConvLSTM** | 3D Tensor | Convolution ($*$) | Forget, Input, Output Gate / **3D Grid 보존** | **시간 차원의 장기 의존성과 공간 차원의 국소성을 동시에 반영** |
