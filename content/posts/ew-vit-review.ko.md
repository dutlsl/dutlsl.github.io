...
> **논문 정보**
> - **제목:** Frequency-Domain Refinement of Vision Transformers for Robust Medical Image Segmentation under Degradation
> - **저자:** Sanaz Karimijafarbigloo, Sina Ghorbani Kolahi, Reza Azad, Ulas Bagci, Dorit Merhof
> - **소속:** University of Regensburg, Tarbiat Modares University, Northwestern University, Fraunhofer MEVIS
> - **학회:** WACV 2025
...
Vision Transformer의 셀프 어텐션 블록에 **웨이블릿 분해**를 통합하여 저주파·고주파 성분을 적응적으로 정제하고, **프롬프트 기반 고주파 정제기(PGHFR)**와 **대조 학습**을 결합하여 노이즈·블러 등 다양한 열화 조건 하에서도 Synapse, ISIC2018, ACDC 데이터셋에서 SOTA 성능을 달성한 의료 영상 분할 프레임워크.
...
의료 영상 분할(Medical Image Segmentation)은 정밀 진단, 치료 계획, 질병 모니터링의 핵심 과정이다. CNN 기반 방법(특히 U-Net)은 괄목할 성과를 거뒀지만 **장거리 의존성 모델링에 한계**가 있다. Vision Transformer(ViT)는 셀프 어텐션을 통해 전역 문맥 정보를 포착하지만, **지역 특징(local feature) 표현 능력이 부족**하다는 구조적 단점이 존재한다.

### 2.2. 기존 방법의 한계

ViT의 지역 특징 부족을 보완하기 위해 CNN-Transformer 하이브리드 아키텍처(TransUNet, HiFormer, CoTr 등)가 제안되었으나, 다음과 같은 한계를 지닌다:

- **높은 파라미터 수와 연산 비용**: TransUNet은 96M 파라미터와 88.91G FLOPs로 비효율적이다.
- **CNN 백본 의존성**: HiFormer, CoTr 등은 여전히 CNN 백본에 크게 의존하며, 다중 스케일 정보의 통합이 불충분하다.
- **고주파 성분 포착 실패**: 연구에 따르면 셀프 어텐션은 **저역 통과 필터(low-pass filter)** 역할을 하여 고주파 성분(경계, 세밀한 텍스처 등)을 충분히 포착하지 못한다.
- **열화 조건 미고려**: 기존 주파수 기반 방법(Laplacian-Former, WaveFormer)도 고주파 정보를 유지하지만, 의료 영상에 흔한 **노이즈·블러·헤이즈** 등의 열화가 고주파 성분을 오염시키는 상황을 처리하지 못한다.

### 2.3. 핵심 기여

1. **웨이블릿 기반 주파수 도메인 모델링**: 셀프 어텐션 내에서 웨이블릿 분해를 수행하여 공간 차원을 절반으로 줄이면서 다중 스케일 표현을 동시에 포착하고, 연산 복잡도를 감소시킨다.
2. **PGHFR 모듈 도입**: 학습 가능한 프롬프트를 활용해 열화 관련 정보를 암묵적으로 인코딩하고, 이를 기반으로 고주파 성분을 동적으로 정제하여 노이즈 효과를 제거한다.
3. **대조 학습 기반 특징 일관성**: 원본과 열화-증강 특징 간의 대조 학습을 통해 표현 공간의 일관성을 유지하고, PGHFR 모듈에 간접적 가이던스를 제공한다.
4. **열화 강건성 검증**: 가우시안 노이즈, 블러, 색상 지터링 등 다양한 열화 유형에 대한 포괄적 어블레이션 스터디를 통해 모델의 강건성을 입증한다.
...
EW-ViT는 U자형 아키텍처로 구성되며, **인코더**, **디코더**, 그리고 두 개의 헤드(**Contrastive Head**, **Segmentation Head**)로 이루어진다.

- **인코더**: 3개 레이어로 구성되며, 각 레이어는 EW-ViT 블록 뒤에 패치 임베딩(다운샘플링 + 차원 확장)이 따른다.
- **디코더**: 3개 레이어로 구성되며, EW-ViT 블록(PGHFR 포함) 뒤에 패치 확장(업샘플링)이 따른다.
- **Contrastive Head**: 인코더 병목(bottleneck) 레이어의 출력으로 대조 학습을 수행한다.
...
2D 특징 맵 $X \in \mathbb{R}^{H \times W \times D}$에 선형 변환을 적용하여 채널을 1/4로 축소한 뒤 ($\widetilde{X} = XW_d$), **이산 웨이블릿 변환(DWT)**을 적용한다. 고전적인 Haar 웨이블릿을 사용하여 저역/고역 필터를 행·열 방향으로 순차 적용하면, 4개의 서브밴드가 생성된다:

- $X_{LL}$: 저주파 성분 — 객체의 기본 구조를 조대(coarse) 수준에서 포착
- $X_{LH}, X_{HL}, X_{HH}$: 고주파 성분 — 세밀한 텍스처, 경계 정보를 보존

각 서브밴드는 공간 해상도가 원본의 절반($H/2 \times W/2$)이므로, 이후 셀프 어텐션의 **연산량이 자연스럽게 감소**한다.
...
별도로 $[X_L, X'_H]$에 **역이산 웨이블릿 변환(IDWT)**을 적용하여 원래 해상도의 정제된 특징을 복원한다. 이 IDWT 경로의 출력과 멀티헤드 어텐션 출력을 결합하여 최종 EW-ViT 블록의 출력을 구성한다:
...
PGHFR은 고주파 성분에서 열화(노이즈, 블러)의 영향을 제거하는 핵심 모듈이며, **Component Prompt Generation (CPG)**과 **Implicit Prompt Interaction (IPI)** 두 서브 모듈로 구성된다.
...
1. $X_H$에 **Global Average Pooling(GAP)**과 **Global Max Pooling(GMP)**을 적용하여 공간 정보를 압축한다.
2. 두 풀링 결과를 합산하고 **Point-Wise Convolution(PWC)** + Softmax를 거쳐 프롬프트 가중치 $w \in \mathbb{R}^{D'}$를 생성한다.
...
인코더 병목 레이어에서 고주파 성분에 **랜덤 열화 노이즈** $G(X_H)$를 주입한 뒤 IDWT로 복원하여 증강 특징 $X^{aug}$를 생성한다. 원본 특징과 증강 특징에서 동일 공간 위치의 픽셀을 샘플링하여 대조 학습 후보로 사용한다.
...
| 데이터셋 | 유형 | 규모 | 분할 대상 |
|---------|------|------|----------|
| **Synapse** | 복부 CT | 30 케이스, 3,779 슬라이스 | 8개 장기 (비장, 신장 등) |
| **ISIC2018** | 피부경 영상 | — | 피부 병변 |
| **ACDC** | 심장 MRI | 100 스캔 (train 70/val 10/test 20) | RV, Myo, LV |

- **옵티마이저**: Synapse — SGD (lr=0.05, momentum=0.9), ISIC/ACDC — Adam (lr=0.0001)
- **손실 함수**: Synapse/ISIC — BDOU + 대조 손실, ACDC — CE + DICE + 대조 손실
- **GPU**: RTX 3090 1장
...
EW-ViT는 33.35M 파라미터, 6.03G FLOPs로 TransUNet 대비 **파라미터 약 1/3, FLOPs 약 1/15** 수준이면서도 DSC를 **6.02%p** 향상시켰다. 기존 주파수 기반 방법인 Laplacian-Former(81.90%)와 WaveFormer(81.92%) 대비 각각 **1.61%p, 1.59%p** 우위를 보인다.
...
ACDC 데이터셋 기준, EW-ViT는 모든 열화 유형과 강도에서 **가장 적은 성능 하락($\Delta$)**을 보였다. 특히 색상 지터링 $\gamma$=1.75 조건에서 MT-UNet은 14.90%p 하락한 반면, EW-ViT는 6.77%p 하락에 그쳤다.
